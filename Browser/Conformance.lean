...
/-- Extract the Page/Action ownership carried by async action callbacks. A
    mismatching ActionId is legal to observe (late callbacks happen) but it must
    be a semantic no-op. -/
private def actionOwnerOf : RuntimeEvent → Option (PageId × ActionId)
  | .local { page := page, kind := .automationFinished action } => some (page, action)
  | .local { page := page, kind := .deadlineArmed action _ _ } => some (page, action)
  | .local { page := page, kind := .deadlineReached action _ } => some (page, action)
  | .local { page := page, kind := .protocolWaitStarted action _ } => some (page, action)
  | .local { page := page, kind := .protocolResponse action _ } => some (page, action)
  | _ => none

private def verifyStaleActionNoop (before after : Model) (event : RuntimeEvent) : Except String Unit := do
  match actionOwnerOf event with
  | none => pure ()
  | some (page, action) =>
      if (before.page page).currentAction != some action then
        if after.page page == before.page page then
          pure ()
        else
          throw "stale action event mutated page"
      else
        pure ()

/-- Verify one Driver record against causality, reaction depth, runtime safety,
    ActionId generation, and the rule that stale action-owned callbacks are
    complete PageState no-ops. -/
def verifyRecord
  ...
  verifyStaleActionNoop m next record.envelope.event
  match record.envelope.event with
  | .local event =>
      match event.kind with
      | .automationStarted =>
          match record.declaredAction with
          | none => throw "actionStarted missing action"
          | some declared =>
              if (next.page event.page).currentAction = some declared then
                pure next
              else
