...
**Do not feed only `batch.features` into the fixed-length training/model path and declare variable-length support complete.** Both classifiers now expose explicit [padding-aware model methods](padded-models.md) that consume the complete batch and preserve observed missing frames; the integration smoke also exercises these methods. The existing training/evaluation loop has not yet been upgraded to carry lengths and masks. The next gate is padding-aware development training/evaluation and the real four-arm runner. These interfaces do not change model equations, graph/control fingerprints, training/rewiring budgets, seeds, success thresholds or final-test authorization. Authorized NTU bytes, independently approved pretrained-weight provenance and the complete runtime/data freeze remain outstanding.
