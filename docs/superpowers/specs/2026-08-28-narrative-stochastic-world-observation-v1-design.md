...
V1 instead uses deterministic, stateless, versioned hash derivation. Each stochastic component gets its own derived stream hash from canonical semantic inputs. A component consumes exactly one categorical draw in V1.
...
`WorldState` gains an optional `root_seed` field appended after the existing fields. Its serialized form is omitted when `None`, preserving deterministic V1 payloads and hashes.

Every next world state must inherit the exact root seed from its prior state.

### 3.2 Versioned derivation

Define a closed implementation constant:

`RANDOM_DERIVATION_VERSION = "narrative-hash-categorical-v1"`

For each sample, construct a canonical derivation payload containing:
...
Derivation is exact and versioned:

1. `stream_hash = stable_content_hash(derivation_payload)`;
2. `draw_hash = stable_content_hash({"derivation_version": RANDOM_DERIVATION_VERSION, "stream_hash": stream_hash, "draw_index": 0})`;
3. remove the `sha256:` prefix from `draw_hash`, take its first 16 hexadecimal digits, and parse them as unsigned `draw_u64`;
4. use `draw_u64 / 2**64` as the categorical variate;
5. walk the lexically canonicalized outcomes in order, selecting the first interval whose cumulative probability is strictly greater than the variate; if none of the non-final intervals matches, select the final outcome.

This algorithm uses no process-global RNG, no mutable per-trajectory RNG, and no dependence on sibling component call order.
...
The constructor must recompute `stream_hash` and `draw_u64` from its declared inputs and reject forged lineage.

The generic record does not independently reconstruct a domain-specific distribution. The enclosing world/observation stochastic-sample record must additionally validate that the selected outcome exists in the declared distribution, that the distribution hash matches, and that the selected outcome hash matches the resulting state delta or observation-fact tuple.
...
- `math.fsum(probabilities)` must equal exactly `1.0`;
- outcomes are canonicalized lexically by outcome id;
- input outcome order must not affect distribution hash or sampling result;
- duplicate outcome ids fail closed;
- distribution payloads are immutable and content-hashed.

V1 does not silently renormalize malformed distributions.

For stochastic spec parameter maps:

- keys must be unique non-empty trimmed strings;
- values must be finite numeric values, excluding bools;
- numeric values are canonicalized to floats;
- key order is canonicalized lexically;
- parameter maps are frozen before hook execution and included in spec identity.
...
`parameters` makes stochastic-model assumptions such as failure probabilities explicit model identity rather than hidden mutable hook instance state.
...
`WorldTransitionModelSpec` gains an optional `stochastic_transitions` tuple appended after the existing `conflict_resolver` field so current positional construction remains compatible.
...
A model may contain only deterministic transitions, only stochastic transitions, or a mixture, subject to cross-lane action-type uniqueness.
...
4. call the distribution hook with a frozen parameter map;
...
Conflict detection therefore operates on realized writes, not the union of every possible stochastic outcome.
...
It receives the same capability-limited immutable world views as the deterministic projection hook, plus a frozen explicit parameter map, and no RNG.
...
`ObservationProjectionModelSpec` gains optional `stochastic_projections` appended after the existing deterministic `projections` field.
...
A model may mix deterministic and stochastic observer/channel specs, subject to cross-lane uniqueness.
...
6. derive one observer/channel-local observation sample from the exact source world-step hash;
7. select one outcome;
...
The seed is validated once and passed to both initial world-state construction and runtime-evidence-ledger construction so they bind the same seeded initial world hash.

`world_state_from_story(..., at_time=None, seed=None)` gains the optional seed that becomes `WorldState.root_seed`.

`runtime_evidence_ledger_from_story(..., at_time=None, seed=None)` gains the same optional seed only to reconstruct the exact seeded initial world state; the ledger does not introduce an independent mutable seed field.

Because the ledger already binds `initial_world_state_hash` and `current_world_state_hash`, seed identity is transitively part of the ledger chain on seeded branches.
