...
displace an earlier nonpositive value. The public regression RED failed two of ten
kernel tests for `(0, True)` and `(-1, 1.0)`; reverse `(True, 0)` remained the
passing precedence control. The minimal ordered per-entry positivity fix passed all
ten kernel tests and all 57 focused contracts, kernel, replay and conformance tests.
