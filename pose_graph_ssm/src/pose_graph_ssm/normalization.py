...
def _interpolate_joint(positions: np.ndarray, usable: np.ndarray, joint: int) -> np.ndarray:
    ...
    for axis in range(3):
        result[:, axis] = np.interp(frames, valid.astype(np.float64, copy=False), positions[valid, joint, axis])
    return result


def normalize_body(body: NtuBody, *, scale_epsilon: float = 1e-6) -> np.ndarray:
    ...
    if positions.ndim != 3 or positions.shape[1:] != (25, 3) or positions.shape[0] <= 0:
    ...
    completed = np.empty_like(positions)
    for joint in range(25):
        completed[:, joint] = _interpolate_joint(positions, usable, joint)

    torso = completed[:, _TORSO] - completed[:, _ROOT]
    ...
    positive = torso_lengths[torso_lengths > epsilon]
    if positive.size == 0:
    ...
    centered = completed - completed[:, _ROOT : _ROOT + 1]
