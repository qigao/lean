...
from .ntu_skeleton import NtuSkeletonBody
...
def _interpolate_joint(
    ...
    result = np.empty((positions.shape[0], positions.shape[2]), dtype=np.float64)
    for coordinate in range(positions.shape[2]):
...
def normalize_ntu_body(
    body: NtuSkeletonBody,
    *,
    scale_epsilon: float = _SCALE_EPSILON,
) -> np.ndarray:
    """Return deterministic, tracking-aware normalized NTU joint trajectories.

    Tracking state 0 and absent-body frames are treated as missing observations.
    States 1 and 2 are admissible. Missing coordinates are filled independently
    for each joint/coordinate by linear interpolation in time; leading/trailing
    gaps use the nearest admissible observation, matching ``numpy.interp``.

    The completed sequence is centered on NTU joint 0 independently per frame and
    divided by the median distance between joints 0 and 20.  This removes global
    translation and uniform subject-scale differences without using labels or
    held-out task outcomes.
    """

    if type(body) is not NtuSkeletonBody:
        raise ValueError("body must be an NtuSkeletonBody")
    ...
    if positions.shape[0] <= 0:
        raise ValueError("NTU body must contain at least one frame")
